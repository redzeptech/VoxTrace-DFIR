...
set "ENABLE_TI=%VOXTRACE_THREAT_INTEL%"
set "ENABLE_REPORT=%VOXTRACE_REPORT%"
...
if /I "!ENABLE_TI!"=="1" (
  if not "%VT_API_KEY%"=="" (
    >> "%LOG%" echo [TI] VirusTotal lookup (Outputs=!OUTDIR!)
    set "TIJSON=%LOGDIR%\threat_intel_!BASE!_%TS%.json"
    python "%~dp0threat_intel.py" "!OUTDIR!" --out "!TIJSON!" >> "%LOG%" 2>&1
  ) else (
    >> "%LOG%" echo [WARN] Threat Intel enabled but VT_API_KEY not set; skipping
  )
)

if /I "!ENABLE_REPORT!"=="1" (
  if exist "%~dp0reporting_engine.py" (
    set "PDF=%LOGDIR%\report_!BASE!_%TS%.pdf"
    >> "%LOG%" echo [REPORT] Generating PDF: !PDF!
    if defined TIJSON if exist "!TIJSON!" (
      python "%~dp0reporting_engine.py" "!OUTDIR!" --case "!BASE!" --run-log "%LOG%" --threat-intel-json "!TIJSON!" --out "!PDF!" >> "%LOG%" 2>&1
    ) else (
      python "%~dp0reporting_engine.py" "!OUTDIR!" --case "!BASE!" --run-log "%LOG%" --out "!PDF!" >> "%LOG%" 2>&1
    )
  ) else (
    >> "%LOG%" echo [WARN] reporting_engine.py not found; skipping report
  )
)
